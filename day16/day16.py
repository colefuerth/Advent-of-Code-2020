...
rules_raw, myticket_raw, others_raw = [
...
valid_others = [other for other in others if all(
    [is_valid_field(o, rules) for o in other])]


possibles = {}
for name, bounds in rules.items():
    possibles[name] = [i for i in range(len(rules)) if all(
        [is_valid_field(t[i], {name: bounds}) for t in valid_others])]
...
dep_prod = product([x for i, x in enumerate(
    myticket) if "departure" in matched[i]])
